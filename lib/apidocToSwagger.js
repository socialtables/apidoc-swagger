...
function createPostPushPutOutput(verbs, definitions, pathKeys) {
	var pathItemObject = {};
	var verbDefinitionResult = createVerbDefinitions(verbs, definitions, pathKeys);
	...
	pathItemObject[verbs.type] = {
		...
		deprecated: verbs.deprecated ? true : undefined,
		"x-admin-only": verbs.permission?.some(permission => permission.name === 'x-admin-only') ? true : undefined, //custom socialtables header
		consumes: [
			"application/json"
		],
		produces: [
...
function createVerbDefinitions(verbs, definitions, pathKeys) {
	pathKeys = pathKeys || [];
	...
	if (verbs && verbs.parameter && verbs.parameter.fields) {
		...
		let nonPathParms = verbs.parameter.fields.Parameter.filter((p) => {
			return !pathKeys.includes(p.field);
		})

		fieldArrayResult = createFieldArrayDefinitions(nonPathParms, definitions, verbs.name, defaultObjectName);		
...
function createPathParameters(verbs, pathKeys) {
	...
	if (verbs.parameter && verbs.parameter.fields.Parameter) {
		...
		for (var i = 0; i < verbs.parameter.fields.Parameter.length; i++) {
			...
			var type = param.type;
			const place = (pathKeys.includes(field)) ? 'path' : 'query';

			pathItemObject.push({
				name: field,
				in: type === "file" ? "formData" : place,
	...
	const headers = verbs.header && verbs.header.fields && verbs.header.fields.Header || [];
